! Ruiling Lu, 2026-06-13:
! Change: Revised plant2soil() so mortality-derived woody carbon enters
!         size-class-specific CWD pools instead of direct SOC transfer.
!---------------
module esdvm
 use datatypes
 use io_mod
...
!==========================================================================
subroutine vegn_demographics_annual(vegn, deltat, n_yr)
  type(vegn_tile_type), intent(inout) :: vegn
  ...
  ! Ruiling, 2026-06-11: starvation is an independent carbon-status mortality
  ! process and can operate together with either size or default mortality.
  if (do_starvation) call vegn_annual_starvation(vegn)
  call vegn_nat_mortality(vegn, deltat, n_yr)
  call vegn_reproduction(vegn)
  ! Ruiling, 2026-07-03: keep the seedling/adult density filters controlled by
  ! namelist flags in the explicit-CWD branch, matching the non-CWD size-mortality
  ! source. This lets calibration test seedling self-thinning while leaving the
  ! broader PFT density cap off unless explicitly requested.
  if (do_seedling_self_thinning) call vegn_seedling_self_thinning(vegn)
  if (do_sapling_self_thinning) call vegn_sapling_self_thinning(vegn)
  if (do_pft_density_cap) call vegn_pft_density_cap(vegn)
#endif
  ! Cohort management
  ! Modified by: Ruiling Lu, 2026-06-12
  ! Xia-style order: first relayer, then merge similar/low-density cohorts,
  ! then remove cohorts whose density was set to zero by merge_cohorts().
...
subroutine vegn_growth(vegn)
  ...
  do i = 1, vegn%n_cohorts
    ...
    associate (sp => spdata(cc%species))
      ...
      ! Ruiling, 2026-06-12:
      ! Revised reproductive carbon allocation for shrub PFTs.
      ! Original code allowed seed production only for mature layer-1 cohorts.
      ! Here, mature shrub cohorts in lower layers are allowed to allocate
      ! a small fraction of surplus growth to seeds, so that shrub PFTs do not
      ! lose regeneration completely after being pushed below the top canopy.
      if(cc%age > sp%AgeRepro)then
        if(cc%layer == 1)then
              dSeed = sp%v_seed * max(0.0, Cgrowth - G_LFR)
              dBSW  = (1.0 - sp%v_seed) * max(0.0, Cgrowth - G_LFR)
        elseif(cc%species == 4 .or. cc%species == 5)then
              dSeed = 0.25 * sp%v_seed * max(0.0, Cgrowth - G_LFR)
              dBSW  = max(0.0, Cgrowth - G_LFR) - dSeed
         else
              dSeed = 0.0
              dBSW  = max(0.0, Cgrowth - G_LFR)
          endif
      else
           dSeed = 0.0
           dBSW  = max(0.0, Cgrowth - G_LFR)
...
subroutine fetch_CN_for_growth(cc,Cgrowth,Ngrowth)
  ...
  associate ( sp => spdata(cc%species) )
    ...
    if (cc%status == LEAF_ON) then ! growing season
        ...
        ! daily growth withdrawal is capped at 2% of the current storage pool
...
subroutine vegn_tissue_turnover(vegn)
  ...
  integer :: i
   ! c Flux
  real :: fall1_dBL1, fall1_dBL2, fall1_dBR, fall1_dBS
  ...
  do i = 1, vegn%n_cohorts
     ...
     ! c FLUX
     cc%fall1_dBL1=dBL * cc%nindivs 
     cc%fall1_dBL2=dAleaf * LMAmin * cc%nindivs 
     cc%fall1_dBR=dBR * cc%nindivs 
     cc%fall1_dBS=dBStem * cc%nindivs 
...
subroutine vegn_daily_starvation (vegn)
  ...
  real :: deadtrees, dead_starv ! number of trees that died over the time step
  integer :: i, k
  type(cohort_type), pointer :: cc
...
subroutine Seasonal_fall(cc,vegn)
  ...
  real    :: fall2_dBL1, fall2_dBL2, fall2_dBR, fall2_dBS
  ...
  if(cc%status == LEAF_OFF .AND. cc%bl > 0.0)then
     ...
     lossN_fine   = (1.-retransN)* cc%nindivs * (dNR        + dAleaf * sp%LNbase)
     !fall2
     cc%fall2_dBL1 = dBL * cc%nindivs
     cc%fall2_dBL2 = dAleaf * LMAmin * cc%nindivs
     cc%fall2_dBR = dBR * cc%nindivs
     cc%fall2_dBS = dBStem * cc%nindivs
  ...
  if (newcohorts >= 1) then   ! build new cohorts for seedlings
     ...
     do i = 1, newcohorts
        ...
        associate (sp => spdata(reproPFTs(i)))
          ...
          ! Ruiling, 2026-06-11: apply PFT-specific germination and
          ! establishment probabilities before creating the new cohort.
          ...
          ! Return seeds that fail to establish to the soil organic pools.
...
function cohort_can_reproduce(cc); logical cohort_can_reproduce
  ...
  logical :: can_seed_layer

  associate (sp => spdata(cc%species) )! F2003
  can_seed_layer = (cc%layer == 1 .or. cc%species == 4 .or. cc%species == 5)
  ! Ruiling, 2026-06-12:
  ! Revised reproduction eligibility for shrub PFTs.
  ! Original code required all reproductive cohorts to be in layer 1.
  ! Here, shrub PFTs are also allowed to reproduce from lower layers,
  ! provided that they are mature and have sufficient seed C and N.

  ! cohort_can_reproduce = (cc%layer == 1 .and. &

  cohort_can_reproduce = (can_seed_layer .and. &
                          cc%nindivs > 0.0 .and. &
...
! ============================================================================
!------------------------Mortality------------------------------------
subroutine vegn_nat_mortality (vegn, deltat,n_yr)
  ...
  do i = 1, vegn%n_cohorts
     ...
     cc%dead_nature =  min(cc%nindivs * cc%mu * deltat/seconds_per_year,cc%nindivs)
     cc%nindivs_ys = cc%nindivs
     !write(*,*)"deadtrees:", deadtrees1,deadtrees
...
subroutine vegn_annual_starvation (vegn)
  ...
  real :: deathrate ! mortality rate, 1/year
  real :: deadtrees ! number of trees that died over the time step
  real :: nsc_ratio, reserve_threshold, maximum_mortality
  integer :: i
  type(cohort_type), pointer :: cc
  ...
  do i = 1, vegn%n_cohorts
     cc => vegn%cohorts(i)
     associate ( sp => spdata(cc%species)  )
       ! Ruiling, 2026-06-11: continuous starvation mortality. Mortality begins
       ! when NSC falls below a configurable fraction of the target leaf pool
       ! and rises linearly to starvation_max_mortality as NSC approaches zero.
       reserve_threshold = starvation_nsc_fraction
       if (starvation_nsc_fraction_pft(cc%species) >= 0.0) &
            reserve_threshold = starvation_nsc_fraction_pft(cc%species)
       reserve_threshold = max(reserve_threshold, 1.0e-6)

       maximum_mortality = starvation_max_mortality
       if (starvation_max_mortality_pft(cc%species) >= 0.0) &
            maximum_mortality = starvation_max_mortality_pft(cc%species)
       maximum_mortality = min(1.0, max(0.0, maximum_mortality))

       nsc_ratio = max(0.0, cc%nsc) / max(cc%bl_max, 1.0e-12)
       deathrate = maximum_mortality * &
                   max(0.0, 1.0 - nsc_ratio / reserve_threshold)

       ! Preserve the original negative-NPP trigger, but make it proportional
       ! rather than deleting the entire cohort in one annual step.
       if (cc%annualNPP < 0.0) deathrate = max(deathrate, &
                                               maximum_mortality)

       deathrate = min(1.0, max(0.0, deathrate))
       deadtrees = min(cc%nindivs, cc%nindivs * deathrate)
       if (deadtrees > 0.0) then
          call plant2soil(vegn,cc,deadtrees)
          cc%nindivs = cc%nindivs - deadtrees
       endif
     end associate
...
!========================================================================
! Ruiling, 2026-06-11: density-dependent self-thinning for small stems.
subroutine vegn_seedling_self_thinning (vegn)
  type(vegn_tile_type), intent(inout) :: vegn

  real :: small_density_ha(0:MSPECIES)
  real :: keep_fraction, deadtrees
  integer :: i, pft
  type(cohort_type), pointer :: cc

  small_density_ha = 0.0
  do i = 1, vegn%n_cohorts
     cc => vegn%cohorts(i)
     if (cc%dbh * 100.0 < seedling_self_thin_dbh_cm) then
        small_density_ha(cc%species) = small_density_ha(cc%species) + &
                                        cc%nindivs * 10000.0
     endif
  enddo

  do pft = 0, MSPECIES
     if (small_density_ha(pft) <= seedling_capacity_ha(pft)) cycle
     keep_fraction = max(0.0, seedling_capacity_ha(pft) / &
                               small_density_ha(pft))

     do i = 1, vegn%n_cohorts
        cc => vegn%cohorts(i)
        if (cc%species /= pft) cycle
        if (cc%dbh * 100.0 >= seedling_self_thin_dbh_cm) cycle

        deadtrees = cc%nindivs * (1.0 - keep_fraction)
        if (deadtrees > 0.0) then
           call plant2soil(vegn,cc,deadtrees)
           cc%nindivs = cc%nindivs - deadtrees
        endif
...
end subroutine vegn_seedling_self_thinning

!========================================================================
! Ruiling, 2026-07-03: soft PFT-specific entrance bottleneck for cohorts at
! or below the 1-cm inventory threshold. We do not have observed DBH < 1 cm
! data, so sapling_capacity_ha is anchored to observed 1 <= DBH < 2 cm
! density and relaxed by sapling_capacity_multiplier before thinning.
subroutine vegn_sapling_self_thinning (vegn)
  type(vegn_tile_type), intent(inout) :: vegn

  real :: small_density_ha(0:MSPECIES)
  real :: effective_capacity_ha, excess_ratio, mortality_fraction, deadtrees
  integer :: i, pft
  type(cohort_type), pointer :: cc
  ...
  do i = 1, vegn%n_cohorts
     cc => vegn%cohorts(i)
     if (cc%dbh * 100.0 <= sapling_self_thin_dbh_cm) then
        small_density_ha(cc%species) = small_density_ha(cc%species) + &
                                        cc%nindivs * 10000.0
     endif
  enddo
  ...
  do pft = 0, MSPECIES
     effective_capacity_ha = sapling_capacity_ha(pft) * &
                             sapling_capacity_multiplier(pft)
     if (effective_capacity_ha <= 0.0) cycle
     if (small_density_ha(pft) <= effective_capacity_ha) cycle

     excess_ratio = small_density_ha(pft) / effective_capacity_ha - 1.0
     mortality_fraction = 1.0 - exp(-sapling_self_thin_strength(pft) * &
                                     excess_ratio)
     mortality_fraction = Min(0.95, Max(0.0, mortality_fraction))

     do i = 1, vegn%n_cohorts
        cc => vegn%cohorts(i)
        if (cc%species /= pft) cycle
        if (cc%dbh * 100.0 > sapling_self_thin_dbh_cm) cycle

        deadtrees = cc%nindivs * mortality_fraction
        if (deadtrees > 0.0) then
           call plant2soil(vegn,cc,deadtrees)
           cc%nindivs = cc%nindivs - deadtrees
        endif
...
end subroutine vegn_sapling_self_thinning


!========================================================================
! Ruiling, 2026-06-11: PFT-level density-dependent self-thinning after the
! small-stem filter. This preserves PFT coexistence under strong competition.
subroutine vegn_pft_density_cap (vegn)
  type(vegn_tile_type), intent(inout) :: vegn

  real :: pft_density_ha(0:MSPECIES)
  real :: excess_indivs, deadtrees, smallest_dbh
  integer :: i, pft, smallest_index
  type(cohort_type), pointer :: cc

  pft_density_ha = 0.0
  do i = 1, vegn%n_cohorts
     cc => vegn%cohorts(i)
     pft_density_ha(cc%species) = pft_density_ha(cc%species) + &
                                  cc%nindivs * 10000.0
  enddo

  do pft = 0, MSPECIES
     if (pft_density_ha(pft) <= pft_density_capacity_ha(pft)) cycle
     excess_indivs = (pft_density_ha(pft) - &
                       pft_density_capacity_ha(pft)) / 10000.0

     ! Remove the smallest cohorts first so density regulation does not
     ! proportionally erase the adult BA and AGB pools.
     do while (excess_indivs > 1.0e-12)
        smallest_index = 0
        smallest_dbh = huge(1.0)
        do i = 1, vegn%n_cohorts
           cc => vegn%cohorts(i)
           if (cc%species /= pft .or. cc%nindivs <= 0.0) cycle
           if (cc%dbh < smallest_dbh) then
              smallest_dbh = cc%dbh
              smallest_index = i
           endif
        enddo

        if (smallest_index == 0) exit
        cc => vegn%cohorts(smallest_index)
        deadtrees = min(cc%nindivs, excess_indivs)
        call plant2soil(vegn,cc,deadtrees)
        cc%nindivs = cc%nindivs - deadtrees
        excess_indivs = excess_indivs - deadtrees
     enddo
  enddo

end subroutine vegn_pft_density_cap
...
subroutine plant2soil(vegn,cc,deadtrees)
  ...
  ! Ruiling Lu, 2026-06-13:
  ! Change: Local variables for routing woody mortality by DBH size class.
  real :: dbh_cm
  integer :: cwd_class

  associate (sp => spdata(cc%species))
     ! Ruiling Lu, 2026-06-13:
     ! Change: Route woody mortality through explicit coarse woody debris pools.
     ! Fine tissues still enter the existing litter pools directly, while
     ! stem/wood tissues enter size-class-specific CWD pools before delayed
     ! decomposition transfers C/N to SOC and Rh.
     ! Ruiling, 2026-06-14: Only stem sapwood and heartwood are coarse
     ! woody debris. The previous expression routed leaf C and leaf N above
     ! their minimum-area amounts into CWD, inflating CWD input and stock.
     ! Route all leaf, root, reserve, and seed material to fine litter while
     ! preserving the total mortality-derived C and N.
     loss_coarse  = max(0.0, deadtrees * (cc%bHW + cc%bsw))
     loss_fine    = max(0.0, deadtrees * (cc%bl + cc%br + cc%nsc + cc%seedC))
     lossN_coarse = max(0.0, deadtrees * (cc%woodN + cc%sapwN))
     lossN_fine   = max(0.0, deadtrees * (cc%leafN + cc%rootN + cc%seedN + cc%NSN))

     dbh_cm = cc%dbh * 100.0
     if (dbh_cm < 10.0) then
        cwd_class = 1
     elseif (dbh_cm < 30.0) then
        cwd_class = 2
     else
        cwd_class = 3
     endif
     ...
     ! Fine mortality input enters existing litter/SOC entry pools directly.
     ...
     vegn%SON(2) = vegn%SON(2) + (1.0 - fsc_fine) * lossN_fine

     ! Ruiling, 2026-06-14: Paired model-structure experiment. The direct
     ! pathway reproduces the original model structure using the same corrected
     ! tissue partition and the same fsc_wood allocation parameters. The
     ! explicit pathway delays wood transfer through size-dependent CWD pools.
     if (use_explicit_cwd) then
        vegn%CWD_C(cwd_class) = vegn%CWD_C(cwd_class) + loss_coarse
        vegn%CWD_N(cwd_class) = vegn%CWD_N(cwd_class) + lossN_coarse
        vegn%CWD_input_yr = vegn%CWD_input_yr + loss_coarse
     else
        vegn%SOC(1) = vegn%SOC(1) + fsc_wood * loss_coarse
        vegn%SOC(2) = vegn%SOC(2) + (1.0 - fsc_wood) * loss_coarse
        vegn%SON(1) = vegn%SON(1) + fsc_wood * lossN_coarse
        vegn%SON(2) = vegn%SON(2) + (1.0 - fsc_wood) * lossN_coarse
     endif

     ! annual N from plants to soil/deadwood pools
     vegn%N_P2S_yr = vegn%N_P2S_yr + lossN_fine + lossN_coarse

     vegn%mort_fine   = vegn%mort_fine   + loss_fine
     vegn%mort_coarse = vegn%mort_coarse + loss_coarse
  end associate
end subroutine plant2soil

!============================================================================
real function mortality_rate(cc,n_yr) result(mu) ! per year
  ...
  !-------local var -------------
  integer :: n,n_yr ! the latest ring
  integer :: mortality_block, mortality_pft
  ...
  real :: alpha_small_use, alpha_large_use
  real :: m0_a_use, m0_b_use, m0_c_use
...
! Ruiling, 2026-06-11: use PFT-specific size-mortality parameters and an
! optional spinup period for the small- and large-tree modifiers.
if (do_U_shaped_mortality) then
    if (n_yr <= size_mort_spinup_years) then
        alpha_small_use = 0.0
        alpha_large_use = 0.0
    else
        alpha_small_use = sp%alpha_small
        alpha_large_use = sp%alpha_large
    end if

    expD = cc%dbh * 100.0   ! convert m to cm

    if (expD <= 1.0) then
        ! Prevent log(0): treat very small trees as 1 cm
        expD = 1.0
    endif

    ! Ruiling, 2026-06-11: the PFT GLMM uses only a + b*DBH + c*log(DBH).
    ! The alpha_small and alpha_large terms were used only for an earlier test.
    ! Ruiling, 2026-07-06: for 500 subplot experiments, switch from baseline
    ! PFT mortality to subplot-specific dominant-PFT mortality after year 160.
    m0_a_use = sp%m0_a
    m0_b_use = sp%m0_b
    m0_c_use = sp%m0_c
    if (use_random_mortality_schedule .and. &
        n_yr >= mortality_schedule_start_year) then
        mortality_block = (n_yr - mortality_schedule_start_year) / &
                          mortality_schedule_interval_years + 1
        if (mortality_block >= 1 .and. &
            mortality_block <= mortality_schedule_nblocks) then
            mortality_pft = mortality_schedule_pft(mortality_block)
            if (cc%species == mortality_pft) then
                m0_a_use = mortality_schedule_a(mortality_block)
                m0_b_use = mortality_schedule_b(mortality_block)
                m0_c_use = mortality_schedule_c(mortality_block)
            endif
        endif
    else if (use_mortality_switch .and. n_yr > mortality_switch_year) then
        m0_a_use = switch_m0_a(cc%species)
        m0_b_use = switch_m0_b(cc%species)
        m0_c_use = switch_m0_c(cc%species)
    endif
    m_S = 1.0 - exp(-exp( m0_a_use                         &
                         + m0_b_use * expD                  &
                         + m0_c_use * log(expD) ))
    mu_bg = Min(0.999, Max(size_mortality_floor(cc%species), m_S)) ! per year

    ! if (expD < 1.0) then
    !     ! For seedlings < 1 cm DBH, use the original background mortality
    !     ! with layer and seedling-size effects, avoiding GLMM extrapolation.
    !     m_S = 1.0 - exp(-exp(sp%m0_a + sp%m0_b))
    !     mu_bg = Min(0.2, sp%r0mort_c * (1.0 + f_L*f_S))
    ! else
    !     ! For trees >= 1 cm DBH, use the fitted PFT size-mortality curve.
    !     m_S = 1.0 - exp(-exp( sp%m0_a        &
    !                          + sp%m0_b * expD &
    !                          + sp%m0_c * log(expD) ))
    !     ! mu_bg = Min(0.999, m_S)
    !     mu_bg = Min(0.2, Max(size_mortality_floor(cc%species), m_S))
    ! endif
else
    mu_bg = Min(0.5, sp%r0mort_c * (1.d0 + f_L*f_S))
endif
...
subroutine initialize_soil(vegn)
   ...
   ! Ruiling, 2026-06-14: Initialize explicit deadwood pools rather than
   ! forcing every CWD experiment to begin from an empty pool.
   vegn%CWD_C(:)  = max(0.0, init_CWD_C(:))
   vegn%CWD_N(:)  = max(0.0, init_CWD_N(:))
...
! Modified by: Ruiling Lu
! Date       : 2026-06-12
! Change     : Xia-style merge. This routine no longer rebuilds the cohort
!              array directly. It merges cohort j into cohort i and leaves the
!              merged cohort with nindivs = 0.0. The following
!              kill_lowdensity_cohorts() call removes zero-density cohorts.
subroutine vegn_mergecohorts(vegn)
  ...
  do i = 1, vegn%n_cohorts
     ...
     ! Go through remaining cohorts and merge them into the current cohort
...
! Modified by: Ruiling Lu
! Date       : 2026-06-12
! Change     : Use min_nindivs as the cleanup threshold for the Xia-style
!              post-merge cleanup. Cohorts that cannot be retained are sent
!              to plant2soil().
subroutine kill_lowdensity_cohorts(vegn)
  ...
  real, parameter :: mindensity = min_nindivs
  integer :: i, j, k
...
! Modified by: Ruiling Lu
! Date       : 2026-06-12
! Change     : c1 is now intent(inout), so merged cohorts can be marked with
!              c1%nindivs = 0.0 and removed by kill_lowdensity_cohorts().
subroutine merge_cohorts(c1,c2) ! Put c1 into c2
  ...
  if(c1%nindivs > 0.0 .and. c2%nindivs > 0.0)then
  ...
  ! update number of individuals in merged cohort
     ...
     ! Zero c1 density so it can be removed by kill_lowdensity_cohorts().
     c1%nindivs = 0.0
...
! Modified by: Ruiling Lu
! Date       : 2026-06-12
! Change     : Replaced old cohorts_can_be_merged() with Xia-style
!              Mergeable_cohorts(). If MergeLowDensityCohorts is defined at
!              compile time, low-density cohorts can merge with same-species,
!              same-layer cohorts even when DBH difference is larger than diff_S0.
     ...
     do i = 1, newcohorts
        ...
        do n=1, vegn%n_initialCC
            if(vegn%initialCC(n)%species == missingPFTs(i))then
               ...
               cc = vegn%initialCC(n)
               ! Ruiling, 2026-06-11: use a PFT-specific immigrant density
               ! instead of one fixed value for every missing PFT.
               cc%nindivs = migration_nindivs_pft(cc%species)
               exit
            endif
        enddo
