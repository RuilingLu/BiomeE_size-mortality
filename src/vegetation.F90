...
real function mortality_rate(cc) result(mu) ! per year
  ...
  associate ( sp => spdata(cc%species))
    ...
    ! Size effect on the mortality of adult trees ruiling
    if(do_U_shaped_mortality)then
      !m_S = 0.0447 + -0.0187*log(cc%dbh*100)+ 0.00255*(cc%dbh*100)
      m_S = exp(m0_a + m0_b*log(cc%dbh*100)+ m0_c*(cc%dbh*100))
    else
       m_S = 0.0
    endif
  ...
  ! Return mortality rate:
     ...
     mu_bg = Min(0.999,Max(0.001, m_S)) ! per year
